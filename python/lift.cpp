...
static InstTransResult LiftInstIntoBlock(TranslationContext &ctx,
										 llvm::BasicBlock *&block,
										 bool doAnnotation, bool add_breakpoints, bool add_tracer) {
...
static bool LiftBlockIntoFunction(TranslationContext &ctx, bool ignore_unsupported_insts, bool add_breakpoints, bool add_tracer) {
	auto didError = false;
	...
	for (auto inst : ctx.natB->get_insts()) {
		ctx.natI = inst;
		switch (LiftInstIntoBlock(ctx, curLLVMBlock, true, add_breakpoints, add_tracer)) {
			case ContinueBlock:
				break;
			case EndBlock:
			case EndCFG:
			...
			case TranslateErrorUnsupported:
				didError = !ignore_unsupported_insts;
				goto done;
			case TranslateError:
				didError = true;
				goto done;
...
static bool InsertFunctionIntoModule(NativeModulePtr mod,
									 NativeFunctionPtr func, llvm::Module *M, bool ignore_unsupported_insts, bool add_breakpoints, bool add_tracer) {
	auto &C = M->getContext();
	...
	for (auto block_info : func->get_blocks()) {
		...
		error = LiftBlockIntoFunction(ctx, ignore_unsupported_insts, add_breakpoints, add_tracer) || error;
	}
...
static bool LiftFunctionsIntoModule(NativeModulePtr natMod, llvm::Module *M, bool ignore_unsupported_insts, bool add_breakpoints, bool add_tracer) {
	// populate functions
	for (auto &func_info : natMod->get_funcs()) {
		NativeFunctionPtr f = func_info.second;
		if (!InsertFunctionIntoModule(natMod, f, M, ignore_unsupported_insts, add_breakpoints, add_tracer)) {
...
bool LiftCodeIntoModule(NativeModulePtr natMod, llvm::Module *M, bool ignore_unsupported_insts, bool add_breakpoints, bool add_tracer) {
	...
	return LiftFunctionsIntoModule(natMod, M, ignore_unsupported_insts, add_breakpoints, add_tracer);
}